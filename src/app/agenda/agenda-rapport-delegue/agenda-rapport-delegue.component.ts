...
@Component({
  selector: "ms-agenda-rapport-delegue",
  templateUrl: "./agenda-rapport-delegue.component.html",
  styleUrls: ["./agenda-rapport-delegue.component.scss"],
})
export class AgendaRapportDelegueComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.get_app_params();
    setTimeout(() => {
      this.pageTitleService.setTitle("Rapport Agenda Délégué");
    }, 0);
  ...
  getUtilisateurs() {
    this.listeCodifService.getUtilisateurs().subscribe(
      (res: any) => {
        ...
        // this.initForm();
        this.form.controls.id_utilisateur.setValue(
          this.listeUtilisateurs.length
            ? this.listeUtilisateurs[0].code_codification
            : null
        );

        this.form.controls.id_utilisateur.valueChanges.subscribe((value) => {
          this.initCalendar();
    ...
    this.calendarOptions = {
      ...
      views: {
        workDaysView: {
          ...
          slotMinTime:
            String(this.hoursRangeStart).length == 1
              ? "0" + this.hoursRangeStart + ":00"
              : this.hoursRangeStart + ":00",
          slotMaxTime:
            String(this.hoursRangeEnd).length == 1
      ...
      eventClick: (info) => {
        if (info.event.extendedProps.delegues) {
          this.openDialogMoreInfo(
            info.event.extendedProps.type_event == "visite"
              ? info.event.extendedProps.delegues[0].visites
              : info.event.extendedProps.delegues[0].evenements,
            info.event.extendedProps.type_event == "visite"
              ? "visites"
              : "evenements"
          );
    ...
    if (viewType == "timeGridWeek" || viewType == "workDaysView") {
      ...
      evenementsByDate.forEach((ev) => {
        let eventIndex = events.findIndex(
          (e) => e.code_evenement == ev.code_evenement && e.date == ev.date
        );
        if (eventIndex != -1) {
      ...
      evenementsByDate.forEach((ev) => {
        let eventIndex = events.findIndex(
          (e) => e.date == ev.date && e.code_evenement == ev.code_evenement
        );
        if (eventIndex != -1) {
          events[eventIndex].title = events[eventIndex].title + 1;
          let delIndex = events[eventIndex].delegues.findIndex(
  ...
  getCalendarEquipeData(startDate, endDate, viewType) {
    ...
    this.agendaService.getAgendaEquipe(body).subscribe(
      (res: any) => {
        ...
        this.makeCalendarEquipeData(
          this.evenements.filter((e) =>
            codesEvenement.includes(e.code_evenement)
          ),
          this.visites.filter((v) => codesVisite.includes(v.code_type_visite)),
          this.calendarComponent.getApi().view.type
        );
      ...
      ...evenements.map((e) => {
        ...
        return {
          ...
          start: isAllDay
            ...
            ? moment(e.date_fin_evenement).add(1, "days").format("YYYY-MM-DD")
            ...
            .subscribe(
              (res: any) => {
                ...
                this.makeCalendarEquipeData(
                  ...
                  this.visites.filter((v) =>
                    codesVisite.includes(v.code_type_visite)
                  ),
                  this.calendarComponent.getApi().view.type
                );
